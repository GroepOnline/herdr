...
pub fn encode_terminal_key(key: TerminalKey, protocol: KeyboardProtocol) -> Vec<u8> {
    ...
    let kitty_flags = match protocol {
        KeyboardProtocol::Kitty { flags } => Some(flags),
        KeyboardProtocol::Legacy => None,
    };
    // Prefer CSI-u when the pane asks for report-all (or event-typed releases)
    // so associated-text and event suffixes are not swallowed by text encoding.
    let prefer_kitty = kitty_flags.is_some_and(|flags| {
        flags & KITTY_FLAG_REPORT_ALL_KEYS != 0
            || (key.kind == crossterm::event::KeyEventKind::Release
                && flags & KITTY_FLAG_REPORT_EVENT_TYPES != 0)
    });
...
mod tests {
    ...
    #[test]
    fn kitty_shifted_punctuation_release_uses_csi_u() {
