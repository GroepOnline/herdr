...
fn parse_integration_target(
    ...
    let Some(target) = args.first().map(|arg| arg.as_str()) else {
        eprintln!(
            "usage: herdr integration {action} <pi|omp|claude|codex|copilot|devin|droid|kimi|opencode|kilo|hermes|qodercli|qwen|cursor|mastracode|antigravity-cli|grok|commandcode|freebuff>"
        );
        return Ok(None);
    };
    if args.len() != 1 {
    ...
    let parsed = match target {
        ...
        "freebuff" => IntegrationTarget::Freebuff,
        _ => {
