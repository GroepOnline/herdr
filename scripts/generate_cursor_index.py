...
try:
    import yaml
except ModuleNotFoundError:  # Keep the control-plane checker stdlib-only.
    yaml = None


class FrontmatterError(ValueError):
    """Raised when the small supported frontmatter subset is malformed."""


YAML_ERROR = yaml.YAMLError if yaml is not None else FrontmatterError
...
def _parse_frontmatter_scalar(value: str) -> Any:
    ...
    if len(value) >= 2 and value[0] == value[-1] == '"':
        # The repository's frontmatter uses single-line JSON-compatible
        # double-quoted values. Keep this fallback intentionally narrow.
        try:
            return json.loads(value)
        except json.JSONDecodeError as exc:
            raise FrontmatterError(f"invalid double-quoted frontmatter value: {value}") from exc
    if len(value) >= 2 and value[0] == value[-1] == "'":
...
def _parse_simple_frontmatter(block: str) -> dict[str, Any]:
    """Parse the top-level scalar fields needed by the index.

    Cursor frontmatter may contain nested routing metadata. The index only
    consumes top-level ``name`` and ``description``; nested keys are skipped
    rather than pretending this small fallback is a complete YAML parser.
    """
...
def parse_frontmatter(path: Path) -> dict[str, Any]:
    ...
    return _parse_simple_frontmatter(parts[1])
...
def _yaml_scalar(value: str) -> str:
    if not value or value[0] in "-?:,[]{}#&*!|>'\"%@`" or value.strip() != value:
...
def _safe_dump_index(payload: dict[str, Any]) -> str:
    lines = [
        f"version: {payload['version']}",
        f"generated_by: {_yaml_scalar(payload['generated_by'])}",
        "items:",
    ]
    for item in payload["items"]:
        lines.append(f"- name: {_yaml_scalar(item['name'])}")
        for key in ("path", "kind", "summary", "when"):
            lines.append(f"  {key}: {_yaml_scalar(str(item[key]))}")
    return "\n".join(lines) + "\n"
...
def write_index_yaml(path: Path, items: list[IndexItem], allow_org_leak: bool) -> None:
    ...
    # Keep generated bytes independent of whether optional PyYAML is installed.
    # This makes `--check` reproducible on developer machines and CI alike.
    rendered = _safe_dump_index(payload)
