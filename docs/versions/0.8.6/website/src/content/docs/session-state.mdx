...
`herdr update --handoff` only applies to direct installs managed by Herdr's own updater. Homebrew, npm, and mise installs run their package-manager upgrade and cannot perform live handoff. Nix prints upgrade guidance and cannot handoff.
