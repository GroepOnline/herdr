...
does not replace the socket API; it exposes JSON routes for scripts, fleet
 tooling, and Moshi-style remote clients. The clipboard endpoint can mutate state,
so secure the gateway before remote exposure.
...
| `HERDR_SOCKET_PATH` | `HERDR_SOCKET_PATH` must be set when using XDG or named sessions | Herdr server socket |
