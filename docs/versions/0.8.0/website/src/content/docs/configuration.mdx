...
Open the in-app settings shell with the settings keybinding (default `prefix+s`). The left-nav modal surfaces most `config.toml` options across the Theme, UI, Sound, System, Templates, and Integrations tabs. Use `/` to search setting labels, and keep editing `config.toml` or CLI helpers (`herdr channel`, `herdr config check`) when you prefer the file path.
