...
### Added
- Consolidated the v0.8.0 product baseline: versioned release documentation; Grok CLI integration with native session restore; antigravity-cli integration; ASCII IME in prefix mode on Windows; distinct status indicators (`ui.status_indicators = "dots" | "symbols"`); Windows remote attach to Unix hosts; marketplace star history with dated backups and delta stats; Windows support for all agent integrations (`.ps1` assets for cursor, devin, grok, mastracode); per-pane right-click routing (`herdr pane split --right-click herdr|pane`).
...
- Website builds now hard-fail on stale legacy product references; archived doc content and config-reference data are rewritten to GroepOnline/herdr.chefgroep.nl at build time.
