...
Herdr keeps the CLI/API command-log view intentionally bounded for fast live
inspection. Plugin command lifecycle metadata is also persisted under
`$XDG_STATE_HOME/herdr/plugin-history/<session>/` (or
`~/.local/state/herdr/plugin-history/<session>/` by default). `commands.jsonl`
contains metadata-only lifecycle records and rotates at 8 MiB; `summary.json`
keeps per-plugin success/failure totals and latest status. Command arguments,
stdout/stderr bodies, error text, pane contents, and environment values are not
written to this persistent history.
...
The example cookbook repo is `GroepOnline/herdr-plugin-examples`. It contains
...
Toggle the bar in **Settings → Plugins** (`ui.fleet_ops_bar`).

The in-repo scaffold `com.chefgroep.ops` documents the contract; production
inventory belongs in [GroepOnline/herdr-ops](https://github.com/GroepOnline/herdr-ops).
See [Fleet ops and gateway](/docs/fleet-ops/) for `HERDR_HOST_NAME`, the
`herdr-gateway` HTTP surface, and `/v1/ops/context`.
