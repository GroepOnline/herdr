...
Working indicators are presentation-only. Choose a spinner from the built-in catalog and choose the status language independently. Herdr ships animated dots, static Unicode symbols, a high-contrast signal set, and an ASCII fallback:

```toml
[ui]
spinner_style = "pulse"
status_indicators = "signal"
```

The Settings surface previews spinner styles before saving. The `focus` pack adds `static`, `pulse`, `bars`, `braille-wave`, `comet`, and `orbit` for compact operational use. Choose `status_indicators = "signal"` when shape needs to carry more meaning than color, or `"ascii"` for limited terminals and fonts. Status icons remain understandable when animation is paused. No spinner changes the underlying agent state or notification authority.
