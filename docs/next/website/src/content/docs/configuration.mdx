...
Open the in-app settings shell with the settings keybinding (default `prefix+s`). The left-nav modal surfaces most `config.toml` options across Theme, UI, Sound, System, Templates, and Integrations. Use `/` to search setting labels, and keep editing `config.toml` or CLI helpers (`herdr channel`, `herdr config check`) when you prefer the file path.
...
The plugin action palette lists every installed plugin action, favorites first.
It opens on `prefix+e` unless you bind `prefix+e` to another action (a
configured `[keys]` action or `[[keys.command]]` entry takes precedence, and
`edit_scrollback` defaults to `prefix+e`). Pin actions to the top and chain
actions together in `config.toml`:

```toml
[plugins]
favorites = ["com.chefgroep.github-status.check-pr"]

[[plugins.chains]]
when = "com.chefgroep.github-status.check-pr"
then = ["com.chefgroep.kater-bridge.sync"]
```

`favorites` lists qualified action ids (`plugin_id.action_id`) shown first in
the palette. `chains` run every id in `then` after `when` finishes successfully;
chained ids use the same qualified format and can themselves have chains (cycles
are detected and skipped). From the palette, `f` toggles a favorite and `b`
records a keybinding for the selected action (written as a `[[keys.command]]`
entry with `type = "plugin_action"`).
...
The sidebar is the main Herdr dashboard. Search `ui.` in the [Config reference](/docs/config-reference/) for sizing, collapsed mode, Agent panel ordering, mouse behavior, pane borders, motion, and other presentation settings.

### Status semantics and motion

Herdr keeps runtime facts and presentation choices separate. The server/runtime determines the pane's observed agent state; the client projects that fact into the same status vocabulary everywhere:

- `working` — active processing;
- `blocked` — input or approval is required;
- `done` — an unseen transition into an idle prompt;
- `idle` — an acknowledged idle prompt;
- `unknown` — insufficient evidence to claim another state.

`unknown` is intentionally not rendered as `idle`. If a status seems wrong, inspect the authority and evidence with `herdr agent explain <target>` rather than inferring from a color alone.

Working indicators are presentation-only. Choose a spinner from the built-in catalog and choose semantic dots or symbols independently:

```toml
[ui]
spinner_style = "dots-full"
status_indicators = "symbols"
```

The Settings surface previews spinner styles before saving. Use a restrained style such as `dots`, `arc`, or `circle` for dense sidebars; reserve emoji and wide multi-cell frames for a panel with enough width. Status icons must remain understandable when animation is paused, when the terminal cannot render a glyph, and for users who prefer reduced visual motion. No spinner changes the underlying agent state or notification authority.
...
Right-side tab bar status is configurable with an ordered list that renders right-aligned in the desktop tab bar. The status area yields to tab controls on narrow rows and hides entries that fail validation.
...
tab_bar_right = [
  ...
  { type = "text", text = "prod" },
  { type = "command", command = "git branch --show-current", interval_seconds = 5, timeout_seconds = 2 },
]
...
- `zoom` — shows `zoom` when the active tab has a zoomed pane, otherwise hidden.
- `hostname` — shows the server hostname.
- `datetime` — shows local wall-clock time. `format` is a strftime string (default `"%H:%M"`). Supports `%Y`, `%m`, `%d`, `%H`, `%M`, `%S`, `%y`, `%p`, `%I`, `%%` and rejects `%z`/`%s`/`%Z` which require UTC. Empty or unsupported formats hide the entry and emit a diagnostic.
- `text` — shows literal `text`.
- `command` — runs `command` on the server and shows its stdout (first line, up to 80 chars, sanitized). `interval_seconds` (1..31536000, default 5) controls poll frequency, `timeout_seconds` (1..3600, default 2) caps execution. Empty commands, out-of-range intervals/timeouts, or failing commands hide the entry. Commands run asynchronously, are killed on reload/cancellation, and their process trees are reaped.

`tab_bar_right_separator` (default single space) is inserted between visible entries. Diagnostics for invalid entries are reported at startup and on `herdr server reload-config` and visible via `herdr config check`.

At most 16 entries are rendered; extras are ignored with a diagnostic.
