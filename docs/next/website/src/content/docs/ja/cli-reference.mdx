...
`herdr-gateway` はローカル Herdr ソケットの上に小さな HTTP API を公開する別
バイナリです。TUI を開かずに fleet スクリプト、ヘルスチェック、読み取り専用の
ランタイム位置クエリに使います。
...
| 変数 | デフォルト | 目的 |
| --- | --- | --- |
| `HERDR_GATEWAY_PORT` | `7777` | 待ち受けポート（localhost） |
| `HERDR_SOCKET_PATH` | 既定 Herdr ソケット | 対象サーバー |

ルートには `/health`、`/v1/ops/context`、`/v1/agents`、`/v1/workspaces`、
`/v1/session`、SSE `/v1/events` があります。ops 契約とホスト ID（`HERDR_HOST_NAME`）は
