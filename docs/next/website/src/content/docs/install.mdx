...
`herdr update` updates the binary that is first on `PATH`. Direct installs download from the configured channel. Homebrew, npm, and mise installs run their package-manager upgrade instead of failing. Nix prints the Nix upgrade path.

If a leftover `~/.local/bin/herdr` is first on `PATH` and a package-manager `herdr` is later on `PATH`, `herdr update` renames the leftover to `herdr.direct.bak` so the package-manager binary wins. Use `herdr update --force-direct` only when you want to keep updating that leftover file.

`herdr --version` prints the version, binary path, and install kind (`direct`, `homebrew`, `npm`, `mise`, or `nix`). `herdr channel` with no subcommand prints the configured channel.
