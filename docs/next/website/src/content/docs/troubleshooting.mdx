...
Update the outer terminal to a version containing the relevant terminal fix:
...
This is especially common with older terminal packages from long-term-support Linux distributions. See [Herdr issue #1116](https://github.com/GroepOnline/herdr/issues/1116) for the confirmed boundary captures and compatibility notes. If the problem remains on a current terminal version, report the exact terminal version and whether it also happens outside Herdr.
...
Herdr deliberately does not rewrite modified arrows because terminal applications may use Alt+Left and Alt+Right directly. See [Herdr issue #1370](https://github.com/GroepOnline/herdr/issues/1370) for the investigation.
...
## curl or Homebrew installed an older version

Direct installs read `https://herdr.chefgroep.nl/latest.json`. If a fresh
`curl | sh` or an outdated Homebrew formula installs a build older than the
current GitHub release, the live manifest or tap formula may not have caught up
yet.
...
```bash
herdr --version
curl -fsSL https://herdr.chefgroep.nl/latest.json | jq -r .version
```

For direct installs, rerun the installer or `herdr update` after the manifest
updates. For Homebrew, use
`brew upgrade GroepOnline/tap/groeponline-herdr` from the
[GroepOnline tap](https://github.com/GroepOnline/homebrew-tap). See
[Install Herdr — Release manifest](/docs/install/#release-manifest).
