...
<CardGrid>
  <Card title="Agents and state">
    See supported agents, detection evidence, integrations, direct attach, and the explicit `working`, `blocked`, `done`, `idle`, and `unknown` states. `unknown` is never silently shown as idle.
  ...
  <Card title="Configuration and motion">
    Configure keybindings, themes, sidebar rows and tokens, status indicators, spinner style, notifications, scrollback, and advanced options. Settings previews are designed to stay responsive without changing server-owned runtime facts.
