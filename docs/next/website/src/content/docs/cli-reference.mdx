...
herdr update                  # update this install (direct download or package manager)
herdr update --handoff        # opt into live handoff for supported running servers
herdr update --force-direct   # keep updating a leftover direct binary that shadows a package manager
herdr completion zsh          # generate a zsh completion script
herdr channel                 # print stable, preview, or dev
herdr channel show            # same as herdr channel
herdr channel set preview     # write preview into config; run herdr update after
herdr channel set dev         # write dev into config; run herdr update after
herdr channel set stable      # write stable into config; run herdr update after
herdr --version               # print version; binary path and install kind go to stderr
```
...
`herdr-gateway` is a separate binary that exposes a small HTTP API on top of the
local Herdr socket. Use it for fleet scripts, health checks, and read-only
runtime location queries without opening the TUI.

`herdr-gateway` runs in the foreground and does not exit, so start it in its own
terminal:
...
curl -s http://127.0.0.1:7777/v1/ops/context
```

| Variable | Default | Purpose |
| --- | --- | --- |
| `HERDR_GATEWAY_PORT` | `7777` | Listen port (localhost) |
| `HERDR_SOCKET_PATH` | default Herdr socket | Target server |

Routes include `/health`, `/v1/ops/context`, `/v1/agents`, `/v1/workspaces`,
`/v1/session`, and SSE `/v1/events`. See
[Fleet ops and gateway](/docs/fleet-ops/) for the ops contract and host identity
(`HERDR_HOST_NAME`).
