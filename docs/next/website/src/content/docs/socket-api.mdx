...
`workspace.close` rejects closing a primary workspace while linked-worktree workspaces are open unless its params include `"close_group": true`. Without group intent the handler returns `workspace_group_close_required`. An explicit group close emits one `workspace.closed` event for each workspace it closes.
