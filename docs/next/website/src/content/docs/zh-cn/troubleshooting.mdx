...
长期支持版 Linux 发行版中的旧终端软件包尤其容易出现此问题。已确认的边界捕获和上游链接见 [Herdr issue #1116](https://github.com/GroepOnline/herdr/issues/1116)。如果当前版本仍有问题，请报告准确的终端版本，以及该问题是否也会在 Herdr 外出现。
...
终端应用可能会直接使用 Alt+Left 和 Alt+Right，因此 Herdr 不会改写带修饰键的方向键。调查详情见 [Herdr issue #1370](https://github.com/GroepOnline/herdr/issues/1370)。
...
## curl 或 Homebrew 安装了旧版本

直接安装会读取 `https://herdr.chefgroep.nl/latest.json`。若新的
`curl | sh` 或过时的 Homebrew formula 安装的版本比当前 GitHub 发布更旧，
可能是 live 清单或 tap formula 尚未更新。
...
直接安装请在清单更新后重新运行安装器或 `herdr update`。Homebrew 请使用
`brew upgrade GroepOnline/tap/groeponline-herdr`，并参考
[GroepOnline tap](https://github.com/GroepOnline/homebrew-tap)。
见[安装 Herdr — 发布清单](/zh-cn/docs/install/#发布清单)。
