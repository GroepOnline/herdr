...
示例菜谱仓库是 `GroepOnline/herdr-plugin-examples`。它在子目录中包含
...
`com.chefgroep.*` 下的 CHEF fleet 插件通过向
`$HERDR_PLUGIN_STATE_DIR` 写入 `fleet_ops.json`,为 Fleet Ops Bar 提供可选的
个人上下文。用 `herdr plugin link <path>` 链接本地 scaffold,再通过
`herdr plugin action invoke <action-id> --plugin <plugin-id>` 调用动作。

工作项以 Linear 为准;PR/CI 以 GitHub 为准;运行时健康以主机 / Cloudflare API
为准。不要在 `fleet_ops.json` 中存储密钥。在**设置 → 插件**
（`ui.fleet_ops_bar`）中切换该栏。

仓库内 scaffold `com.chefgroep.ops` 说明契约;生产库存应放在
[GroepOnline/herdr-ops](https://github.com/GroepOnline/herdr-ops)。
`HERDR_HOST_NAME`、`herdr-gateway` HTTP 面与 `/v1/ops/context` 见
[Fleet ops 与 gateway](/zh-cn/docs/fleet-ops/)。
