...
GroepOnline 通过专用 Homebrew tap 发布 Herdr。上游的
`brew install herdr` 公式 formula 可能落后于
...
如果 mise 报 `herdr not found in mise tool registry`,请更新 mise 后重试。旧版 mise 早于 Herdr 的注册表条目;`mise use -g github:GroepOnline/herdr` 可以作为临时兜底。
...
把 `v0.x.y` 替换为最新的发布标签。省略标签可以跟踪 `main`,但常规安装建议使用发布标签。
...
如果你已经在用 Cargo,可以直接从仓库安装 Herdr:

```bash
cargo install --git https://github.com/GroepOnline/herdr.git
```

这会构建当前的 `main` 分支。要安装发布构建,请传入发布标签:

```bash
cargo install --git https://github.com/GroepOnline/herdr.git --tag v0.8.0
```

从源码构建需要 Rust 和 Zig 工具链(见 `rust-toolchain.toml`),比脚本或包管理器安装耗时更长。Cargo 安装通过重新运行同一条命令来更新;`herdr update` 只适用于安装器管理的安装。

## 手动下载

也可以从 [GitHub releases](https://github.com/GroepOnline/herdr/releases) 下载二进制文件。
...
直接安装和远程引导会读取
`https://herdr.chefgroep.nl/latest.json` 上的稳定版清单。它必须与当前
GitHub 发布标签和已发布资产一致。如果新的 `curl | sh` 安装的版本比另一台机器上的
`herdr --version` 更旧,可能是发布后的站点部署尚未更新 live 清单。
...
该 recipe 会比较 `Cargo.toml`、git 标签、GitHub 发布资产、本地
`website/latest.json` 和 live 站点清单。打标签后,用
`python3 scripts/homebrew_formula.py --version X.Y.Z` 重新生成 Homebrew formula,
并向 [GroepOnline/homebrew-tap](https://github.com/GroepOnline/homebrew-tap)
提交 PR。

预览和 dev 直接安装在 `/preview.json` 与 `/dev.json` 上使用相同 schema。
...
在 Linux 和 macOS 上,Herdr 默认使用稳定更新通道。要启用来自 `main` 的预览构建,设置通道:
